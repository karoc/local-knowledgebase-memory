...
class MemoryPlugin {
    ...
    private recentConfirmedWritesById = new Map<number, RecentWriteEntry>();
    private recentConfirmedWritesByExactKey = new Map<string, RecentWriteEntry[]>();

    constructor(api: OpenClawPluginApi) {
        this.api = api;
        this.config = api.pluginConfig as unknown as PluginConfig;
    }
    ...
    private expandAgentAliases(agentId?: string | null): string[] {
        const canonical = this.normalizeAgentId(agentId);
        const aliases = new Set<string>([canonical]);
        if (canonical === 'main') aliases.add('default');
        if (canonical === 'default') aliases.add('main');
        return [...aliases];
    }

    private normalizeProjectId(projectId?: string | null): string | null {
        const value = (projectId || '').trim();
        if (!value) return null;
        return value;
    }

    private expandProjectAliases(projectId?: string | null): string[] {
    ...
    private expandSessionAliases(sessionKey?: string | null): string[] {
    ...
    private appendInFilter(sql: string, field: string, values: string[], queryParams: any[]): string {
    ...
    private normalizeExpiresAt(expiresAt?: string | null): string | null {
    ...
    private registerTools() {
        this.api.registerTool((ctx: OpenClawPluginToolContext) => ({
            name: 'memory_recall',
            label: '回忆记忆',
        ...
        this.api.registerTool((ctx: OpenClawPluginToolContext) => ({
            ...
            execute: async (_toolCallId: string, params: ForgetParams) =>
        ...
        this.api.registerTool((ctx: OpenClawPluginToolContext) => ({
            ...
            execute: async (_toolCallId: string, params: GetParams) =>
                this.handleGet({ ...params, agentId: params.agentId || ctx?.agentId })
        }) as any);
        ...
        this.api.registerTool((ctx: OpenClawPluginToolContext) => ({
            ...
            execute: async (_toolCallId: string, params: UpdateParams) =>
    ...
    private async fetchMemoryById(memoryId: number, agentId?: string): Promise<MemoryRecord | null> {
        const agentAliases = this.expandAgentAliases(agentId);
        const [rows]: any = await this.pool.query(
            `SELECT * FROM memories WHERE id = ? AND agent_id IN (${agentAliases.map(() => '?').join(',')}) LIMIT 1`,
            [memoryId, ...agentAliases]
        );
        if (!Array.isArray(rows) || !rows.length) return null;
        return rows[0] as MemoryRecord;
    }

    private isExactMatchListRequest(params: {
        scope?: string | null;
    ...
    private recentWriteTtlMs(): number {
        return 5000;
    }

    private buildExactMatchCacheKey(entry: {
        agentId: string;
        scope: string;
        projectId?: string | null;
        sessionKey?: string | null;
    ...
    private pruneRecentConfirmedWrites() {
        const now = Date.now();
        for (const [id, entry] of this.recentConfirmedWritesById.entries()) {
            if (entry.expiresAt <= now) this.recentConfirmedWritesById.delete(id);
        }
        for (const [key, entries] of this.recentConfirmedWritesByExactKey.entries()) {
            ...
            if (alive.length) this.recentConfirmedWritesByExactKey.set(key, alive);
            else this.recentConfirmedWritesByExactKey.delete(key);
        }
    }

    private recordRecentConfirmedWrite(entry: {
        memoryId: number;
        agentId: string;
        scope: MemoryScope;
        projectId?: string | null;
    ...
    private getRecentConfirmedWriteById(memoryId: number): RecentWriteEntry | null {
        this.pruneRecentConfirmedWrites();
        return this.recentConfirmedWritesById.get(memoryId) || null;
    }

    private getRecentConfirmedWritesByExactKey(entry: {
        agentId: string;
        scope: string;
        projectId?: string | null;
        sessionKey?: string | null;
    ...
    private rowMatchesExactFilters(row: any, filters: {
        agentAliases: string[];
        scope?: string | null;
        status?: string | null;
        projectAliases: string[];
        ...
        if (!row) return false;
        if (filters.agentAliases.length && !filters.agentAliases.includes(String(row.agent_id || ''))) return false;
        if (filters.scope && String(row.scope || '') !== filters.scope) return false;
        if (filters.status && String(row.status || '') !== filters.status) return false;
        if (filters.projectAliases.length && !filters.projectAliases.includes(String(row.project_id || ''))) return false;
        if (filters.sessionAliases.length && !filters.sessionAliases.includes(String(row.session_key || ''))) return false;
        if (filters.memoryKey && String(row.memory_key || '') !== filters.memoryKey) return false;
        if (filters.source && String(row.source || '') !== filters.source) return false;
        return true;
    }

    private traceTool(event: string, payload: Record<string, any> = {}) {
        try {
    ...
    private async confirmStoredMemory(memoryId: number, expected: {
        agentId: string;
        scope: MemoryScope;
        projectId?: string;
        sessionKey?: string;
    ...
    private async onBeforeStart(event: { prompt: string; runId?: string; messages?: unknown[] }, ctx: { agentId?: string; sessionKey?: string; sessionId?: string; workspaceDir?: string }) {
        try {
            const msg = (event.messages?.[0] as any)?.content || event.prompt || '';
        ...
        } catch (e) { this.api.logger.error('[memory-local] 召回失败: ' + String(e)); }
        return {};
    }

    private async onAgentEnd(event: { messages: unknown[]; success: boolean; error?: string; durationMs?: number }, ctx: { agentId?: string; sessionKey?: string; workspaceDir?: string }) {
        try {
            const msg = (event.messages?.[0] as any)?.content || '';
        ...
        } catch (e) { this.api.logger.error('[memory-local] 捕获失败: ' + String(e)); }
    }
    ...
    async handleGet(params: GetParams) {
        const agentId = this.normalizeAgentId(params.agentId);
    ...
    async handleList(params: ListParams) {
        ...
        const normalizedScope = this.normalizeOptionalString(params.scope as string | undefined);
        const normalizedStatus = this.normalizeOptionalString(params.status as string | undefined);
        const normalizedMemoryKey = this.normalizeOptionalString(params.memoryKey);
        const normalizedSource = this.normalizeOptionalString(params.source as string | undefined);
        const agentAliases = this.expandAgentAliases(agentId);
        ...
        let fallbackMemoryId: number | null = null;
        if (!result.length && exactMatchMode) {
            ...
            if (recentEntries.length) recentCacheHit = true;
            retried = true;
            await this.sleep(30);
    ...
    async handleUpdate(params: UpdateParams) {
        ...
        const [result]: any = await this.pool.execute(
            `UPDATE memories SET ${updates.join(', ')} WHERE id = ? AND agent_id IN (${agentAliases.map(() => '?').join(',')})`,
            values
        );
    ...
    async handleForget(params: ForgetParams) {
        ...
        const agentAliases = this.expandAgentAliases(agentId);
        const [result]: any = await this.pool.execute(
            `UPDATE memories SET valid = 0, status = 'deleted', updated_at = NOW() WHERE id = ? AND agent_id IN (${agentAliases.map(() => '?').join(',')})`,
            [params.memoryId, ...agentAliases]
        );
        if (!result || result.affectedRows === 0) {
